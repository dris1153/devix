...
export function SortModeSelect({ value, onChange }: Props) {
    return (
        <select
            ...
            className="cursor-pointer rounded border border-[#2d2d2d] bg-[#181818] px-2.5 py-1 font-mono text-[11px] tracking-wider text-slate-400 uppercase transition-colors hover:border-[#5d5d5d] hover:text-slate-200 focus:border-primary-foreground/50 focus:ring-1 focus:ring-primary-foreground/30 focus:outline-none"
        >
            {OPTIONS.map((o) => (
